...
import org.opensrp.path.fragment.EditWeightDialogFragment;
import org.opensrp.path.fragment.StatusEditDialogFragment;
...
import org.opensrp.path.tabfragments.ChildRegistrationDataFragment;
import org.opensrp.path.tabfragments.ChildUnderFiveFragment;
import org.opensrp.path.toolbar.ChildDetailsToolbar;
import org.opensrp.path.view.ImmunizationRowGroup;
...
//        getSupportActionBar().
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        for (int i = 0; i < weightlist.size(); i++) {
            ...
            org.opensrp.path.db.Event event = null;
            PathRepository db = (PathRepository) VaccinatorApplication.getInstance().getRepository();
            if (weight.getEventId() != null) {
                event = ecUpdater.convert(db.getEventsByEventId(weight.getEventId()), org.opensrp.path.db.Event.class);
            } else if (weight.getFormSubmissionId() != null) {
                event = ecUpdater.convert(db.getEventsByFormSubmissionId(weight.getFormSubmissionId()), org.opensrp.path.db.Event.class);
            }
            if (event != null) {
    ...
    private void updateProfilePicture(Gender gender) {
        ...
        if (isDataOk()) {
            ...
            if (childDetails.entityId() != null) {//image already in local storage most likey ):
                ...
                profileImageIV.setTag(org.opensrp.R.id.entity_id, childDetails.entityId());
...
//            setLastModified(true);
    ...
    private boolean showVaccineListCheck(String eventId, String formSubmissionId) {
        ...
        org.opensrp.path.db.Event event = null;
        if (eventId != null) {
            event = ecUpdater.convert(db.getEventsByEventId(eventId), org.opensrp.path.db.Event.class);
        } else if (formSubmissionId != null) {
            event = ecUpdater.convert(db.getEventsByFormSubmissionId(formSubmissionId), org.opensrp.path.db.Event.class);
        }
        if (event != null) {
